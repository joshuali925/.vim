...
command! -bang W if '<bang>' == '' | execute 'write !sudo tee %' | else | %yank | vnew | setlocal buftype=nofile bufhidden=wipe nobuflisted noswapfile | 0put='Enter password in terminal and press <\C-u>pa<\Esc>;w' | wincmd p | execute 'lua require("packer").loader("neoterm")' | execute "botright T sudo nvim +'set paste' +'1,$d' +startinsert %" | endif
command! Grt Gcd
