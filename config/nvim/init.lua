...
vim.keymap.set("x", "P", function()
    ...
    vim.cmd.normal({ '"' .. vim.v.register .. "P", bang = true })
    if is_visual_line then vim.cmd.normal({ "`[v`]=", bang = true }) end
end)
...
vim.api.nvim_create_user_command("W", [[call mkdir(expand('%:p:h'), 'p') | if '<bang>' == '' | execute 'write !sudo tee "%" > /dev/null' | else | let [t, p, b] = [tempname(), expand('%:p'), bufnr()] | execute 'silent write! ' . fnameescape(t) | botright 10new | execute 'terminal sudo tee ' . shellescape(p) . ' < ' . shellescape(t) . ' >/dev/null' | execute 'autocmd TermClose <buffer> ++once call delete("' . t . '") | call setbufvar(' . b . ', "&modified", 0) | bdelete!' | startinsert | endif]], { bang = true })
...
if require("states").small_file then
    ...
    vim.defer_fn(function()
        ...
        vim.o.foldtext = ""
        require("pack").load({ "gitsigns.nvim", "unclash.nvim", "nvim-scrollview", "quick-scope" })
