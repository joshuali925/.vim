return {
    ...
    { "tpope/vim-unimpaired", install = false, keys = { { { "n", "x", "o" }, "[" }, { { "n", "x", "o" }, "]" }, { "n", "=p" }, { "n", "yo" } } },
    {
        ...
        keys = {
            ...
            { "n", "q", "<Cmd>lua require('snacks.picker').buffers()<CR>" },
            {
                ...
                function()
                    ...
                    if mc and mc.hasCursors() then mc.deleteCursor() else require("snacks.picker").smart() end
                end,
            },
            {
                "x",
                ...
                dashboard = {
                    ...
                    preset = {
                        ...
                        keys = {
                            ...
                            { icon = "󰏖 ", key = "p", desc = "List plugins (update: 'u', clean: 'x')", action = ":lua require('pack').list()" },
                            { icon = "󰚰 ", key = "u", desc = "Update plugins", action = ":lua vim.pack.update()", hidden = true },
                            { icon = "󰃢 ", key = "x", desc = "Cleanup plugins", action = ":execute 'PackDelete' | lua require('pack').list()", hidden = true },
