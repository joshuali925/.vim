    ...
    { "skywind3000/asyncrun.vim", cmd = "AsyncRun", config = function() vim.g.asyncrun_open = 12 end },
    {
        ...
        config = function()
            ...
            vim.fn["quickui#menu#install"]("&Git", { -- GBrowse loaded on command won't include line number in URL, need to explicitly load it. Similar issues for other fugitive commands.
                ...
                { "Git &changed files", [[execute "lua require('lazy').load({plugins = 'vim-flog'})" | Git! difftool --name-status]], "Load unstaged files into quickfix (Git! difftool --name-status)" },
                { "Git unstaged changes", [[execute "lua require('lazy').load({plugins = 'vim-flog'})" | Git! difftool]], "Load unstaged changes into quickfix (Git! difftool)" },
                { "Git HEAD changes", [[execute "lua require('lazy').load({plugins = 'vim-flog'})" | Git! difftool HEAD]], "Load changes from HEAD into quickfix (Git! difftool HEAD)" },
