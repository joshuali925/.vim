    ...
    vim.api.nvim_create_autocmd("LspAttach", {
        ...
        callback = function(e)
            ...
            vim.keymap.set("n", "K", function()
                if vim.tbl_contains({ "vim", "help" }, vim.o.filetype) then return vim.cmd.normal({ "K", bang = true }) end
            ...
            vim.keymap.set("n", "ms", "<Cmd>lua vim.diagnostic.jump({ count = -1, float = { border = 'single' }, severity = { min = vim.diagnostic.severity[next(vim.diagnostic.get(0, { severity = vim.diagnostic.severity.ERROR })) ~= nil and 'ERROR' or 'HINT'] } })<CR>", opts)
            vim.keymap.set("n", "md", "<Cmd>lua vim.diagnostic.jump({ count = 1, float = { border = 'single' }, severity = { min = vim.diagnostic.severity[next(vim.diagnostic.get(0, { severity = vim.diagnostic.severity.ERROR })) ~= nil and 'ERROR' or 'HINT'] } })<CR>", opts)
            vim.keymap.set("n", "mS", "<Cmd>lua vim.diagnostic.jump({ count = -1, float = { border = 'single' } })<CR>", opts)
            vim.keymap.set("n", "mD", "<Cmd>lua vim.diagnostic.jump({ count = 1, float = { border = 'single' } })<CR>", opts)
