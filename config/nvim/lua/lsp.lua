...
function M.setup()
    ...
    vim.api.nvim_create_autocmd("LspAttach", {
        ...
        callback = function(e)
            ...
            vim.keymap.set("n", "[d", "<Cmd>lua vim.diagnostic.jump({ count = -1, float = { border = 'single' }, severity = { min = vim.diagnostic.severity[next(vim.diagnostic.get(0, { severity = vim.diagnostic.severity.ERROR })) ~= nil and 'ERROR' or 'HINT'] } })<CR>", opts)
            vim.keymap.set("n", "]d", "<Cmd>lua vim.diagnostic.jump({ count = 1, float = { border = 'single' }, severity = { min = vim.diagnostic.severity[next(vim.diagnostic.get(0, { severity = vim.diagnostic.severity.ERROR })) ~= nil and 'ERROR' or 'HINT'] } })<CR>", opts)
            vim.keymap.set("n", "[D", "<Cmd>lua vim.diagnostic.jump({ count = -1, float = { border = 'single' } })<CR>", opts)
            vim.keymap.set("n", "]D", "<Cmd>lua vim.diagnostic.jump({ count = 1, float = { border = 'single' } })<CR>", opts)
