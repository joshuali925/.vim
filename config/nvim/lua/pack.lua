...
local function to_pack_spec(spec)
    return {
        src = spec[1]:match("^https?://") and spec[1] or ("https://github.com/" .. spec[1]),
        name = spec.name,
...
function M.setup()
    local specs_handle = assert(vim.uv.fs_scandir(vim.fn.stdpath("config") .. "/lua/plugins"))
    local pack_handle = vim.uv.fs_scandir(vim.fn.stdpath("data") .. "/site/pack/core/opt")
    local to_install, installed = {}, {}
    local event_queue, load_queue = {}, {}

    if pack_handle then
        while true do
            local entry = vim.uv.fs_scandir_next(pack_handle)
            if not entry then break end
            installed[entry] = true
        end
    end
    while true do
        local name, type = vim.uv.fs_scandir_next(specs_handle)
    ...
    vim.api.nvim_create_autocmd("PackChanged", {
        callback = function(event)
            ...
            if deferred_builds then return table.insert(deferred_builds, spec) end
            spec.build()
        end,
    })

    if #to_install > 0 then vim.pack.add(to_install, { load = false, confirm = false }) end
    for _, spec in ipairs(deferred_builds) do spec.build() end
    for _, spec in ipairs(load_queue) do load_plugin(spec, true) end
    deferred_builds = nil

    vim.api.nvim_create_user_command("PackDelete", function(ev) vim.pack.del({ ev.args }, { force = true }) end, { nargs = 1, complete = function() return vim.tbl_keys(specs) end })
end
