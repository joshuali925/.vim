...
__fzf_cd__() {
  ...
  dir=$(eval "$cmd" | FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} --reverse $FZF_DEFAULT_OPTS $FZF_ALT_C_OPTS" $(__fzfcmd) +m) && if [[ -d "$dir" ]]; then printf 'cd %q' "$dir"; else printf '%q %q < /dev/tty' "$EDITOR" "$dir"; fi
}
