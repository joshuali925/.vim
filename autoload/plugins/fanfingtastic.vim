" https://raw.githubusercontent.com/dahu/vim-fanfingtastic/6d0fea6dafbf3383dbab1463dbfb3b3d1b94b209/plugin/fanfingtastic.vim
...
" Private Functions: {{{1
function! plugins#fanfingtastic#get(var) "{{{2
  return get(s:, a:var, '')
endfunction
...
function! plugins#fanfingtastic#next_char(count, char, f, fwd) "{{{2
...
function! plugins#fanfingtastic#visual_next_char(count, char, f, fwd) "{{{2
  let pos1 = s:get_visual_pos()
  let pos2 = getpos("'<") == pos1 ? getpos("'>") : getpos("'<")
  let vmode = visualmode(1)
  " call setpos('.', pos1)
  let pos3 = [0] + plugins#fanfingtastic#next_char(a:count, a:char, a:f, a:fwd) + [0]
  if pos3[1] == 0
    exec 'normal! gv'
    return ''
  endif
...
function! plugins#fanfingtastic#operator_next_char(count, char, f, fwd) "{{{2
  let curpos = getpos('.')
  ...
  call setpos("'[", curpos)
  call setpos("']", curpos)
  let pos = [0] + plugins#fanfingtastic#next_char(a:count, a:char, a:f, a:fwd) + [0]
  if pos[1] != 0
    call setpos("']", pos)
  endif
...
for [mode, fn_prefix] in [['n', ''], ['x', 'visual_'], ['o', 'operator_']]
  for [cmd, arg1, arg2] in [
    ...
    exec printf(
          ...
          \ . ':<C-U>call plugins#fanfingtastic#%snext_char(v:count1, %s, %s, "%s")<CR>',
