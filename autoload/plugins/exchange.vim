...
function! plugins#exchange#exchange_set(type, ...)
	if !exists('b:exchange')
		...
		if cmp == 'overlap'
			...
			return plugins#exchange#exchange_clear()
		elseif cmp == 'outer'
		...
		call plugins#exchange#exchange_clear()
	endif
endfunction

function! plugins#exchange#exchange_clear()
	unlet! b:exchange
...
nnoremap <silent> <expr> <Plug>(Exchange) ':<C-u>set operatorfunc=plugins#exchange#exchange_set<CR>'.(v:count1 == 1 ? '' : v:count1).'g@'
xnoremap <silent> <Plug>(Exchange) :<C-u>call plugins#exchange#exchange_set(visualmode(), 1)<CR>
nnoremap <silent> <Plug>(ExchangeClear) :<C-u>call plugins#exchange#exchange_clear()<CR>
nnoremap <silent> <expr> <Plug>(ExchangeLine) ':<C-u>set operatorfunc=plugins#exchange#exchange_set<CR>'.(v:count1 == 1 ? '' : v:count1).'g@_'
...
command! -bar XchangeClear call plugins#exchange#exchange_clear()
