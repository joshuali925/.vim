...
" Init global vars
function! plugins#expand_region#init()
...
call plugins#expand_region#init()
...
function! plugins#expand_region#custom_text_objects(...)
  if a:0 == 1
...
" select mode
function! plugins#expand_region#use_select_mode()
  return g:expand_region_use_select_mode || index(split(s:saved_selectmode, ','), 'cmd') != -1
endfunction

" Main function
function! plugins#expand_region#next(mode, direction)
...
function! s:select_region()
  ...
  if plugins#expand_region#use_select_mode()
    exec "normal! \<C-g>"
  endif
endfunction
